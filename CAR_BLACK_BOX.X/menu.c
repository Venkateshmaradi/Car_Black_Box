...
{
	const char *menu[] = {"VIEW LOGS", "DOWNLOAD LOGS", "CLEAR LOGS", "CHANGEPASSWORD", "SET TIME"};
	...
	while (!exit_flag)
	{
		...
		break;
		case MK_SW5: /*Scroll Down*/
		{
			delay(5);
			// long press check
			if (read_switches(LEVEL_CHANGE) == MK_SW5)
				exit_flag = 1;
			else
			{
